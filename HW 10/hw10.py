...
def conf_matrix(y_pred, y_true, num_class):
    ...
    M = np.zeros((num_class, num_class))    #Create a matrix of zeros
    for element in range(len(y_pred)):    #Use predicted and true values to fill in matrix
        M[int(y_pred[element]), int(y_true[element])] += 1
    return M
