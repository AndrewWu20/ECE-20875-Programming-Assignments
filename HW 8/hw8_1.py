...
def get_dict(filename, n):
    ...
    '''
    list_ngrams = []    #Initialize list of n gram
    # 1. get lines and combine them to a set of keys
    #   fill in
    lines = get_formatted_text(filename)    #Read file
    for count in lines:                     #Adds each element of file into a list
        for element in get_ngrams(count, n):
            list_ngrams.append(element)
    # 2. use the set to initialize a dict
    #   fill in
    ngram_dict = dict.fromkeys(list_ngrams, 0)   #Creates a dictionary of ngrams
    # 3. update the values of the dict
    #   fill in
    for counter in list_ngrams: #Finds ngram in lines and unpdates count for each ngram found
        ngram_dict[counter] += 1
...
def top_N_common(filename, N, n):
    ...
    # 1. sort a dict
    # fill in
    last_dictionary = {}    #Initialize dictionaries
    common_N = {}
    first_dictionary = get_dict(filename, n)    #Fill dictionaries
    sort_dictionary = sorted(first_dictionary.items(), key = lambda x: (x[1], x[0]))
    for key, value in sort_dictionary:  #Sift through and move keys starting with space to the end
        if key.startswith(' '):
            last_dictionary[key] = value
        else:
            last_dictionary.update({key: value})
    last_dictionary = dict(reversed(list(last_dictionary.items())))
    # 2. get the top N common n-grams from the sorted dict and return it
    #   fill in
    keys = list(last_dictionary.keys())[:N] #Get most common ngrams from dictionary and return them
    for key in keys:
        common_N[key] = last_dictionary[key]
    return common_N
...
def dict_union(listOfDicts):
    ...
    union_ngrams = set() #Initialize set named union_ngrams
    # 1. update the set by using set.update()
    # fill in
    for dicts_lang in listOfDicts:  #Fill in set with dictionary
        union_ngrams.update(dicts_lang.keys()) 
    # 2. sort the set by converting it to a list and then sort
    # fill in
    union_ngrams = sorted(list(union_ngrams))   #Convert set to list and sort it
...
def compare_langs(test_file, langFiles, N, n=3):
    ...
    # fill in
    cardinalities = {}
    for count in langFiles: #Compare languages in langfiles and finds top N and stores in list for future use
        top_N = top_N_common(count, N, n)
        card_int = len(set(mystery_top_N.keys()).intersection(top_N.keys()))
        cardinalities[count] = card_int
    lang_match = max(cardinalities, key = cardinalities.get)    #Find language with biggest cardinality, return file location
