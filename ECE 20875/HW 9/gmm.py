...
def concatenate_features(X1, X2):
    ...
    # 2. Using the np.reshape() method, reshape X2 to the shape (n, 1)
        # fill in
    X2_reshaped = np.reshape(X2, (len(X2),1))
...
def gaus_mixture(data, n_components):
    ...
    gm = GaussianMixture(n_components = n_components[0], random_state = 0).fit(data)    #Initialize variables
    ...
    for k in n_components:
        ...
        if bic < best_bic:
            best_bic = bic  #Update best bic to current bic of index k
            best_no_clusters = k    #Update best no cluster to value of index k
