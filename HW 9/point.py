...
class Point:
    ...
    def distFrom(self, other):
        ...
        # fill in
        distance = 0  #Initialize and set distance equal to zero
        for element in range(self.dim):
            distance = distance + ((self.coords[element] - other.coords[element]) ** 2) #Distance is sum of squared distances
        distance = math.sqrt(distance)  #Take the square root of the squared sums
        return distance
...
def makePointList(data):
    ...
    """
    # fill in
    points = [] #Initialize  list to contain points
    for element in data:
        points.append(Point(element)) #Append points to list
    return points
